...
class KBAutomatedTester:
    def __init__(self):
        self._url = "http://localhost:5000/searchkb"
        self._payload = """{\"text\" : \"@@@@\",\"client\": \"thameem\",\"user_name\": \"thameem.sakkarai\",
                            \"is_sfb\" : \"N\",\"is_initial_chat\" : \"N\"}"""
        self._headers = {'content-type': "application/json"}
        self._file_path = "Payload.xlsx"
        self._upload_url = "http://localhost:5000/storeinkb"
    ...
    def upload_doc(self):
        ...
        try:
            ...
            for i in os.listdir(self._upload_filepath):
                ...
                if (len(fullText) > 0):
                    d = ''.join(fulltxt).replace('\t', '').replace('\n', '').replace('\"','')
                    d.encode("utf-8", errors="strict")
                    print(d)
                    input_payload = self._upload_payload
                    # input_payload["Body"] = str(str(fullText).encode("utf-8", errors="strict"))
                    # input_payload["Body"] = str(fullText)
                    # input_payload["DocumentTypeExtension"] =str(extension.upper())
                    # input_payload["FileName"] = str(filename)
